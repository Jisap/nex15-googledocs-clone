...
export function useSearchParam<T>(key: string, defaultValue: T) {            // wrapper personalizado para gestionar parámetros de búsqueda en URLs usando la biblioteca nuqs. Toma un key (nombre del parámetro) y un valor por defecto
  return useQueryState(                                                      // Utiliza useQueryState para sincronizar el estado con la URL
    key,                                                                     // Toma un key (nombre del parámetro) y un valor por defecto
